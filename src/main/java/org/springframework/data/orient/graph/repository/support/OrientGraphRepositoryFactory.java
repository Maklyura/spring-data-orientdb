...
public class OrientGraphRepositoryFactory extends RepositoryFactorySupport {

  private final OrientObjectOperations operations;

  public OrientGraphRepositoryFactory(OrientObjectOperations operations) {
    super();
  ...
  protected Object getTargetRepository(RepositoryInformation metadata) {
    ...
    if (isGraphRepository(metadata.getRepositoryInterface())) {
      if (cluster != null) {
        return new SimpleOrientGraphRepository(operations, javaType, cluster, repositoryInterface);
      } else {
        return new SimpleOrientGraphRepository(operations, javaType, repositoryInterface);
      }
    } else {
      if (cluster != null) {
  ...
  protected Class<?> getRepositoryBaseClass(RepositoryMetadata metadata) {
    if (isGraphRepository(metadata.getRepositoryInterface())) {
      return SimpleOrientGraphRepository.class;
    } else {
      return SimpleOrientRepository.class;
    }
  }

  private boolean isGraphRepository(Class<?> repositoryInterface) {
    return OrientGraphRepository.class.isAssignableFrom(repositoryInterface);
  }
